...
Generated by `python -m veritor.stress.report` from `docs/data/stress-control-flow.json`, `docs/data/stress.json`; `U` is `Bound` at `eta = 2^-40`, overhead is `Cost(...).total` over the honest replay cost at the simulation's policy `theta = (1/2, 1/8)`.
...
| C5a | variable-length generation, request RUs: 6 requests, max_new 3..13, EOS stops | M5 in-shape: each request's circuit is its streamed length; no advice | 0 | 576 | 0.889 | 72,128 | no advice: lengths are public (the client sees the stream end), route (c) |
| C5b | variable-length generation, step RUs (ClusterG, 1 pod x 2 slots): the same requests | M4 charged advice: the schedule (Schedule.encode(): a header and 7 words per join) | 1,632 | 2,208 | 1.39 | 76,955 | advice = the schedule; each request's length rides in its join (28 bytes) |
...
- **C5a**: 36 tokens streamed of 48 asked; lengths (1, 5, 7, 3, 11, 9) (3 EOS stops); U = 576 bits (interface-capped); 604.1 gates/token; the length would be output-determined advice (charged 0) if it had to be advice at all.
- **C5b**: U = 576 bits (interface-capped) + 1632 advice bits for 6 joins over 19 steps; 604.1 gates/token (same work, cut into steps); overhead 1.39 vs 0.89 with request RUs: the KV cache crosses every step boundary. The per-request lengths are output-determined; only the placement (pod, slot, step) is genuine advice.
