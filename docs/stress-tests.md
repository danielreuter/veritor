...
|---|---|---|---|---|---|---|---|
| S1a | continual batching, ClusterG (RU = step), fcfs: 5 joins on 2x2 pods x slots | M4 | 95 | 256 | 1.12 | 43,422 | outputs = reference; the schedule is the advice and the step kinds follow it |
| S1b | continual batching, ClusterG (RU = step), reversed admission: 5 joins on 2x2 pods x slots | M4 | 95 | 256 | 1.11 | 43,161 | outputs = reference; the schedule is the advice and the step kinds follow it |
| S1c | continual batching, ClusterG (RU = step), one slot per pod: 5 joins on 2x2 pods x slots | M4 | 95 | 256 | 1.12 | 43,208 | outputs = reference; the schedule is the advice and the step kinds follow it |
...
| S3a | preemption by recompute, ClusterG: request evicted after 2 tokens, re-prefilled 3 steps later on another pod | M4 | 110 | 256 | 1.12 | 43,990 | outputs = reference; both attempts are in the circuit, the recomputed positions declared but not output |
...
| S4a | preemption by swap, ClusterG: KV cache retained across a gap of 3 steps (Schedule v3 resume) | M4 | 110 | 256 | 1.12 | 43,091 | outputs = reference; the resumed decode step reads the KV rows declared 3 steps earlier; honest cost unchanged |
...
| S5a | chunked prefill, ClusterG: a 9-token prompt in 3 chunks of 3 over 3 steps | M4 | 38 | 80 | 1.06 | 44,404 | outputs = reference; the step-RU description differs (chunk kinds), the honest cost does not |
...
| S6a | prefill/decode disaggregation, ClusterG: prefill on pod 0, decode on pod 1 (resume across pods) | M4 | 96 | 144 | 1.07 | 34,370 | outputs = reference; a decode step reads another pod's declared KV rows through its ports |
...
| S7 | client disconnect / abort: 6 requests asked for max_new = 8, the clients received (8, 3, 5, 1, 6, 2) tokens (25 of 48); request RUs, the generated length as advice, the absent slots blank check outputs | M4 charged advice (exact bits) + check outputs for the absent slots; M3 open | 18 | 418 | 0.615 | 58,237 | ACCEPTED; the steps are RequestsG's for requests that asked for t tokens, and the max_new - t absent slots per request are blank (vocab) check outputs: 0 bits, U unchanged |
...
| S8b | retries, ClusterG: the duplicate attempt declared in the schedule | M4 | 110 | 256 | 1.12 | 43,993 | outputs unchanged; the duplicate's steps are replay units, its tokens declared but not output |
...
| C1b | variable-length generation, step RUs (ClusterG, 1 pod x 2 slots): the same requests | M4 charged advice: the schedule (Schedule.encode() v4, bit-packed, exact bits) | 118 | 694 | 1.12 | 76,955 | advice = the schedule; each request's length rides in its join (118 bits for 6 joins, charged exactly; 120 bits on the wire) |
...
| C2b | MoE routing, advised: the same shape and requests; the route is advice, only chosen experts run | M4 charged advice: ceil(log2 E) bits per chosen expert per position per layer, charged exactly; route_check VU folds into ok, a check output (0 bits) | 20 | 132 | 0.616 | 38,397 | the honest server's pick once E/k is large (from E=16, k=1 under the VU-output interior, C2c): k/E of the expert compute for k*log2(E) bits per position, spent on a stronger theta |
| C2c | MoE crossover sweep: (E, k) in {(2,1), (4,1), (8,1), (8,2), (16,1)}, vocab 16, 32 requests x 4 tokens, theta grid q in {1/2, 1}, s in {1/8..7/8} | M5 vs M4 compared at equal theta (equal relative overhead) and at equal absolute prover cost | 0 | 0 | 0 | 0 | at equal theta padding beats advice in U at every (E, k), by exactly \|a\| (the ok words are check outputs, 0 bits); at equal absolute prover cost advice wins from E=16, k=1 on (E=2,k=1:padding, E=4,k=1:padding, E=8,k=1:padding, E=8,k=2:padding, E=16,k=1:advice) |
...
| N3 | heterogeneous fleet, ClusterG: pod 0 on sm80, pod 1 on sm90 (two namespaced copies of the toy ISA in one Σ) | M2 | 96 | 144 | 1.07 | 44,884 | outputs = reference; one circuit over union_gate_set (14 gates: 6 operators x 2 + in, weight); 6 step kinds carry their architecture; prefill on sm80 decoded on sm90; a foreign gate name fails to compile |
| N4 | silent data corruption: bit 0 of one dot product's output word flipped in RU 3 of the small simulation (2 pods x 16 steps, 12 requests, 30 streamed tokens, 26314 gates, 3791 VUs); the server streamed the consequences (0 tokens changed) and finds the VU when it replays the opened RU | M6 | 271 | 751 | 1.12 | 51,367 | f_max = 0: RELATION_REJECTED at the faulty VU once the s-challenge sampled it (it escaped 0 earlier challenges at s = 1/8); f_max = 1: ACCEPTED, declared; the s-challenge did not sample it; a second undeclared corruption under the same budget, everything sampled: RELATION_REJECTED |
| N5 | a GPU produces a wrong token that was streamed: the top bit of a logit dot's output word flips, the sampler draws a different token and the request continues from it (2 and 2 tokens changed in the two runs); outputs stand as streamed | M6 | 271 | 751 | 1.12 | 51,367 | fault in RU 7, not opened by the q-challenge: nothing declared, ACCEPTED (7 of 20 RUs opened); fault in RU 7, opened but VU 1814 not sampled: declared anyway (the server cannot know the s-challenge), ACCEPTED |
...
| W3 | several models on one cluster: three weight sets of one shape, a router assigns six requests | M2 (client chose, model in x: 0 bits) / M4 (server chose: advice, exact bits) | 12 | 396 | 0.616 | 41,684 | both ACCEPTED on the same circuit (same digest); the advised route costs exactly its bits |
| E1 | scale, RequestsG: 1,000,000 requests of 119 shapes (Zipf-like prompts, bimodal generation) | M1 | 0 | 33,713 | 0.609 | 340,162 | description 340,162 B; trace < 10 s, compile < 0.1 s, kind table < 0.1 s, bound < 10 s |
...
- **C2c**: E=2,k=1: gates padded/advised 1.21x (121945 vs 100665), description 34805 vs 45186 bytes, \|a\| 160 b over 128 tokens; at theta=(1/2,1/8) capacity 2048 vs 2208 (overhead 0.611 vs 0.609); at equal absolute cost 112404: padding 2048 vs advice 2208 -> padding \| E=4,k=1: gates padded/advised 1.62x (169761 vs 104961), description 39662 vs 47575 bytes, \|a\| 320 b over 128 tokens; at theta=(1/2,1/8) capacity 2048 vs 2368 (overhead 0.611 vs 0.608); at equal absolute cost 157152: padding 2048 vs advice 2368 -> padding \| E=8,k=1: gates padded/advised 2.40x (273073 vs 113553), description 53295 vs 51767 bytes, \|a\| 480 b over 128 tokens; at theta=(1/2,1/8) capacity 2048 vs 2528 (overhead 0.610 vs 0.607); at equal absolute cost 250969: padding 2048 vs advice 2528 -> padding \| E=8,k=2: gates padded/advised 1.94x (273073 vs 140913), description 53295 vs 57131 bytes, \|a\| 960 b over 128 tokens; at theta=(1/2,1/8) capacity 2048 vs 3008 (overhead 0.610 vs 0.608); at equal absolute cost 250969: padding 2048 vs advice 3008 -> padding \| E=16,k=1: gates padded/advised 3.90x (510417 vs 130737), description 94112 vs 60679 bytes, \|a\| 640 b over 128 tokens; at theta=(1/2,1/8) capacity 2048 vs 2688 (overhead 0.607 vs 0.605); at equal absolute cost 455883: padding 2048 vs advice 1500 -> advice
