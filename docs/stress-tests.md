...
|---|---|---|---|---|---|---|---|
| S1a | continual batching, ClusterG (RU = step), fcfs: 5 joins on 2x2 pods x slots | M4 | 1,408 | 256 | 1.39 | 43,422 | outputs = reference; the schedule is the advice and the step kinds follow it |
| S1b | continual batching, ClusterG (RU = step), reversed admission: 5 joins on 2x2 pods x slots | M4 | 1,408 | 256 | 1.39 | 43,161 | outputs = reference; the schedule is the advice and the step kinds follow it |
| S1c | continual batching, ClusterG (RU = step), one slot per pod: 5 joins on 2x2 pods x slots | M4 | 1,408 | 256 | 1.4 | 43,208 | outputs = reference; the schedule is the advice and the step kinds follow it |
...
| S2a | scale, RequestsG: 1,000 requests of 98 shapes (Zipf-like prompts, bimodal generation) | M1 | 0 | 30,392 | 0.889 | 306,134 | description 306,134 B; trace < 0.1 s, compile < 0.1 s, kind table < 0.1 s, bound < 10 s |
| S2b | scale, RequestsG: 10,000 requests of 119 shapes (Zipf-like prompts, bimodal generation) | M1 | 0 | 31,530 | 0.889 | 339,452 | description 339,452 B; trace < 0.1 s, compile < 0.1 s, kind table < 0.1 s, bound < 10 s |
| S2c | scale, RequestsG: 100,000 requests of 119 shapes (Zipf-like prompts, bimodal generation) | M1 | 0 | 32,435 | 0.889 | 339,808 | description 339,808 B; trace < 1 s, compile < 0.1 s, kind table < 0.1 s, bound < 10 s |
| S3a | preemption by recompute, ClusterG: request evicted after 2 tokens, re-prefilled 3 steps later on another pod | M4 | 1,632 | 256 | 1.4 | 43,990 | outputs = reference; both attempts are in the circuit, the recomputed positions declared but not output |
...
| S4a | preemption by swap, ClusterG: KV cache retained across a gap of 3 steps (Schedule v3 resume) | M4 | 1,632 | 256 | 1.4 | 43,091 | outputs = reference; the resumed decode step reads the KV rows declared 3 steps earlier; honest cost unchanged |
...
| S5a | chunked prefill, ClusterG: a 9-token prompt in 3 chunks of 3 over 3 steps | M4 | 736 | 80 | 1.34 | 44,404 | outputs = reference; the step-RU description differs (chunk kinds), the honest cost does not |
...
| S6a | prefill/decode disaggregation, ClusterG: prefill on pod 0, decode on pod 1 (resume across pods) | M4 | 1,632 | 144 | 1.35 | 34,370 | outputs = reference; a decode step reads another pod's declared KV rows through its ports |
...
| S7 | client disconnect / abort: 6 requests asked for max_new = 8, the clients received (8, 3, 5, 1, 6, 2) tokens (25 of 48); request RUs, the generated length as advice | M4 (charged as advice; M3 open) | 24 | 424 | 0.893 | 57,303 | ACCEPTED; the circuit is byte-identical to RequestsG over requests that asked for t tokens |
...
| S8b | retries, ClusterG: the duplicate attempt declared in the schedule | M4 | 1,632 | 256 | 1.4 | 43,993 | outputs unchanged; the duplicate's steps are replay units, its tokens declared but not output |
...
| C1b | variable-length generation, step RUs (ClusterG, 1 pod x 2 slots): the same requests | M4 charged advice: the schedule (Schedule.encode(): a header and 7 words per join) | 1,632 | 2,208 | 1.39 | 76,955 | advice = the schedule; each request's length rides in its join (28 bytes) |
...
| C2b | MoE routing, advised: the same shape and requests; the route is advice, only chosen experts run | M4 charged advice: ceil(log2 E) bits per chosen expert per position per layer; route_check VU folds into ok | 24 | 168 | 0.896 | 42,473 | the honest server's pick once E/k is large (from E=8, k=1 here, C2c): k/E of the expert compute for k*log2(E) bits per position, spent on a stronger theta |
| C2c | MoE crossover sweep: (E, k) in {(2,1), (4,1), (8,1), (8,2)}, 32 requests x 4 tokens, theta grid q in {1/2, 1}, s in {1/8..7/8} | M5 vs M4 compared at equal theta (equal relative overhead) and at equal absolute prover cost | 0 | 0 | 0 | 0 | at equal theta padding beats advice in U at every (E, k), by \|a\| plus the ok words; at equal absolute prover cost advice wins from E=8, k=1 on (E=2,k=1:padding, E=4,k=1:padding, E=8,k=1:advice, E=8,k=2:padding) |
...
| C3b | speculative decoding, advised: the same models and requests; m per step is advice | M4 charged advice: ceil(log2(gamma+2)) bits per step; acceptance_check VU folds 'exactly m agree' into ok | 24 | 232 | 0.893 | 81,378 | the honest server's pick: the target does exactly plain decoding's positions plus the draft's |
...
| N3 | heterogeneous fleet, ClusterG: pod 0 on sm80, pod 1 on sm90 (two namespaced copies of the toy ISA in one Σ) | M2 | 1,632 | 144 | 1.35 | 44,884 | outputs = reference; one circuit over union_gate_set (14 gates: 6 operators x 2 + in, weight); 6 step kinds carry their architecture; prefill on sm80 decoded on sm90; a foreign gate name fails to compile |
| N4 | silent data corruption: bit 0 of one dot product's output word flipped in RU 10 of the small simulation (2 pods x 16 steps, 12 requests, 30 streamed tokens, 26314 gates, 3791 VUs); the server streamed the consequences (0 tokens changed) and finds the VU when it replays the opened RU | M6 | 3,648 | 4,128 | 1.4 | 51,367 | f_max = 0: RELATION_REJECTED at the faulty VU once the s-challenge sampled it (it escaped 1 earlier challenge at s = 1/8); f_max = 1: ACCEPTED, declared; the s-challenge did not sample it; a second undeclared corruption under the same budget, everything sampled: RELATION_REJECTED |
| N5 | a GPU produces a wrong token that was streamed: the top bit of a logit dot's output word flips, the sampler draws a different token and the request continues from it (1 and 2 tokens changed in the two runs); outputs stand as streamed | M6 | 3,648 | 4,128 | 1.4 | 51,367 | fault in RU 9, not opened by the q-challenge: nothing declared, ACCEPTED (11 of 20 RUs opened); fault in RU 7, opened but VU 1812 not sampled: declared anyway (the server cannot know the s-challenge), ACCEPTED |
...
| W3 | several models on one cluster: three weight sets of one shape, a router assigns six requests | M2 (client chose, model in x: 0 bits) / M4 (server chose: advice) | 16 | 400 | 0.894 | 41,684 | both ACCEPTED on the same circuit (same digest); the advised route costs exactly its bits |
...
- **C1b**: U = 576 bits (interface-capped) + 1632 advice bits for 6 joins over 19 steps; 604.1 gates/token (same work, cut into steps); overhead 1.39 vs 0.89 with request RUs: the KV cache crosses every step boundary. The per-request lengths are output-determined; only the placement (pod, slot, step) is genuine advice.
...
- **C2c**: E=2,k=1: gates padded/advised 1.29x (96017 vs 74171), description 29861 vs 55300 bytes, \|a\| 160 b over 128 tokens; at theta=(1/2,1/8) capacity 2048 vs 2720 (overhead 0.889 vs 0.893); at equal absolute cost 130591: padding 2048 vs advice 2720 -> padding \| E=4,k=1: gates padded/advised 1.86x (143833 vs 77326), description 34718 vs 83330 bytes, \|a\| 320 b over 128 tokens; at theta=(1/2,1/8) capacity 2048 vs 2880 (overhead 0.889 vs 0.893); at equal absolute cost 195660: padding 2048 vs advice 2880 -> padding \| E=8,k=1: gates padded/advised 2.95x (247145 vs 83726), description 48351 vs 90253 bytes, \|a\| 480 b over 128 tokens; at theta=(1/2,1/8) capacity 2048 vs 3040 (overhead 0.892 vs 0.895); at equal absolute cost 333957: padding 2048 vs advice 1384 -> advice \| E=8,k=2: gates padded/advised 2.31x (247145 vs 107027), description 48351 vs 168771 bytes, \|a\| 960 b over 128 tokens; at theta=(1/2,1/8) capacity 2048 vs 3520 (overhead 0.892 vs 0.895); at equal absolute cost 333957: padding 2048 vs advice 3520 -> padding
