...
Generated by `python -m veritor.stress.report` from `docs/data/stress.json`; `U` is `Bound` at `eta = 2^-40`, overhead is `Cost(...).total` over the honest replay cost at the simulation's policy `theta = (1/2, 1/8)`.
...
| S2a | scale, RequestsG: 1,000 requests of 98 shapes (Zipf-like prompts, bimodal generation) | M1 | 0 | 30,392 | 0.889 | 306,134 | description 306,134 B, trace 34 ms, compile 52 ms, kind table 17 ms, bound 1264 ms |
| S2b | scale, RequestsG: 10,000 requests of 119 shapes (Zipf-like prompts, bimodal generation) | M1 | 0 | 31,530 | 0.889 | 339,452 | description 339,452 B, trace 56 ms, compile 57 ms, kind table 20 ms, bound 2499 ms |
| S2c | scale, RequestsG: 100,000 requests of 119 shapes (Zipf-like prompts, bimodal generation) | M1 | 0 | 32,435 | 0.889 | 339,808 | description 339,808 B, trace 247 ms, compile 61 ms, kind table 20 ms, bound 4051 ms |
...
- **C6b**: U capped at \|Out\| = 384 bits (uncapped 428 bits); crossover: route A wins on replay work for k >= 2 requests (saves (k - 1) x 4776) and pays 64 boundary words once; U here: A 384 vs B 384 bits
