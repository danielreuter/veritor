...
> **Corrected Sep 2 (recompute-honest cost model, commit b9c0188; `docs/frontier-report.md` re-run with 12 partitions, 1008 points).** Terminology from here on: replay unit = RU, verification unit = VU. Refinement (a) as stated above was wrong about the cost. The morning's `cost()` priced a sampled RU as $q\times$(its replay cost + hashing its interior), which assumes the RU's *inputs* are available for free. The honest prover retains only the prompts and the weights; a `cell`, `row`, `matvec`, `layer` or decode `step` RU reads activations or the KV cache, and to replay it the prover must re-execute the smallest *closed* kind containing it (`KindSummary.closed`: every port fed by an `in`/`weight` gate at every call site) — the `request` for anything finer than a step, the whole run for a decode step (steps batch 32 requests). With $m$ such RUs per closed kind the re-execution happens with probability $1-(1-q)^m$, which is 1 for every grid $q$ ($m$ from $3\times10^4$ to $10^{10}$): **every RU level finer than `request` costs $\approx100\%$ of the honest computation in recomputation** (`recompute = 1.000` in the data; 0.98/0.99 for `step`/`layer` at $q=1/8192$), so `cell/gate` as an *RU* level has prover overhead $1 + q(0.667+s)$, from 1.0002 to 1.83, never below 1. The `U` column is unchanged. What *is* nearly free is `cell` as a **VU** level under `request` RUs: `request/cell` has the same prover overhead as `request/row`, $q(1.667+s)$, and a far smaller bound once $s<1$ (at $q=1/8$, $s=1/8$, $\eta=10^{-2}$: `request/row` 2,396,735 bits vs `request/cell` 284,048 bits $\approx\Lambda_q\times 8192 = 282{,}522$, the RU channel alone, at overhead 0.224). So the two levers are now: (a′) request RUs with dot-product VUs, and (b) the reach-aware Bound. The table above should be read with a `request/cell` row added (53.5 kbit at $q=1/2,s=1/8$, overhead 0.896) and with `cell/gate`'s cost understood as $\ge1.0$; its 3.93 kbit is available only to a prover paying 140% overhead.
...
> **Corrected Sep 2.** The formula and the code description above are the morning's model and are superseded by commit b9c0188 (`cost.py`, `ExpectedCost(boundary, recompute, commit_interior, proof, weights)`; `.replay` is now the sum `recompute + commit_interior`). The replay term is split: the interior-commitment part stays $q\sum_r h|\mathrm{Int}(r)|$, but the recomputation part is
>
> $$\mathrm{recompute} = \sum_{A\ \text{closed},\ m_A>0} \mathrm{copies}_A\,\bigl(1-(1-q)^{m_A}\bigr)\,\mathrm{replay}(A),$$
>
> where a kind is *closed* if every port, at every call site, is fed by an `in` or `weight` gate (`KindSummary.closed`, computed top-down in `Index.kinds()`), and $m_A$ is the number of RUs whose smallest closed ancestor is $A$ (`recomputation_units(table)`). The retention assumption, stated in the module docstring: the honest prover retains only the circuit inputs and the weights; every other value is regenerated by re-executing the smallest closed kind containing the sampled RU. For a closed RU ($m=1$) the term is $q\cdot\mathrm{replay}$, the old formula, so **"overhead $\approx q(1.667+s)$" holds only for the `request/*` partitions** (all 84 points each: `(overhead/q − s)` = 1.6666). For every finer RU level $q\,m\gg1$ and $\mathrm{recompute}/\text{honest} = 1.000$ (0.9816 for `step` and 0.9942 for `layer` at $q=1/8192$, where $qm\approx4$–$5$), so **prover overhead $\approx 1 + q(0.667+s)$**: 1.396 at $q=1/2,s=1/8$; 1.0002 at $q=1/8192,s=1/512$. The `step` RUs' recomputation unit is the whole run ($m=32{,}704$ decode steps; the 64 `prefill_step`s are closed); for `layer` and finer it is the request's `prefill` ($m$ = 593 to $7\times10^9$) or the `request` ($4\times10^4$ to $10^{10}$). $(1-q)^m$ is computed exactly when $m\cdot\mathrm{bitlen}(\mathrm{den}(q))\le4096$, as 0 when $qm>64$, and as a float otherwise (`cost.survival`, three tested branches).
...
Shape: `vocab=32768, d_model=8192, heads=64, layers=80, prompt=512, generated=512, requests=2048, batch=32, hidden_multiplier=4, width=16`; $n = 2.744\times10^{17}$; honest cost $4.1168\times10^{17}$ (identical for all twelve partitions); inputs $1{,}048{,}576$, outputs $1{,}048{,}576$ ($16{,}777{,}216$ output bits $= 1{,}048{,}576$ generated tokens $\times$ 16), weights $6.496\times10^{10}$.

> **Corrected Sep 2.** The two tables that stood here were computed under the morning's cost model and gave `step/row` and `cell/gate` the same 0.8958 overhead as `request/row`; those numbers were wrong (the `U` values were right). Corrected, from `docs/data/frontier-70b.json` (1008 points, 12 partitions including the new `cell` VU level: `request/cell`, `step/cell`, `layer/cell`, `matvec/cell`, `row/cell`). Per-partition at $q=1/2, s=1/8$:

| partition (RU/VU) | RUs | closed RU kinds | recomputation unit of the open RUs ($m$) | boundary/honest | recompute/honest | commit-interior/honest | proof/honest | total/honest |
|---|---|---|---|---|---|---|---|---|
| `request/row`, `request/cell`, `request/gate` | $2.05\times10^3$ | `weights`, `request` | none | $5.1\times10^{-12}$ | 0.5000 | 0.3333 | 0.0625 | 0.8958 |
| `step/row`, `step/cell` | $3.28\times10^4$ | `weights`, `prefill_step` | the run ($32{,}704$) | $6.7\times10^{-6}$ | 1.0000 | 0.3333 | 0.0625 | 1.3958 |
| `layer/*`, `matvec/*`, `row/*` | $8.7\times10^7$ to $1.24\times10^{13}$ | `weights`, `prompt` (zero cost) | `prefill` / `request` (593 to $3\times10^9$ each) | $\le5.4\times10^{-5}$ | 1.0000 | 0.3333 | 0.0625 | 1.3958 |
| `cell/gate` | $3.47\times10^{13}$ | `weights`, `prompt` | `prefill` ($7.1\times10^9$) / `request` ($9.9\times10^9$) | $8.4\times10^{-5}$ | 1.0000 | 0.3333 | 0.0625 | 1.3959 |

> Headline `certify` results (measured; agree with the re-run `docs/frontier-report.md`; bits per generated token in brackets):

| $\eta$ | overhead $\le 1\%$, work $\le 10\%$ | overhead $\le 10\%$, work $\le 1$ | overhead $\le 1$, work $\le 10$ |
|---|---|---|---|
| $1/2$ | `request/cell` $q=1/512,s=1/8$: $U=2{,}908{,}248$ (17.3%) [2.77/token], overhead 0.0035, work 0.019 | `request/cell` $q=1/32,s=1/8$: $U=180{,}026$ (1.07%) [0.17/token], overhead 0.056, work 0.31 | `request/cell` $q=1/2,s=1/8$: $U=8{,}248$ (0.049%) [0.008/token], overhead 0.896, work 4.96 |
| $1/100$ | nothing non-vacuous (16 Mbit) | same policy: $U=1{,}195{,}659$ (7.1%) [1.14/token] | same policy: $U=54{,}744$ (0.33%) [0.052/token] |
| $10^{-6}$ | nothing non-vacuous | same policy: $U=3{,}586{,}234$ (21.4%) [3.42/token] | same policy: $U=164{,}200$ (0.98%) [0.157/token] |

> `cell/gate` no longer appears under any prover budget $\le1$: its overhead is $\ge1.0002$ everywhere and 1.396 at $q=1/2,s=1/8$, where its $U$ (640 / 4,021 / 11,712 bits) is 13× to 14× below `request/cell`'s. Points at the cap: 802 of 1008 (all 672 points of the eight `step`/`layer`/`matvec`/`row` partitions; 55 of `request/row`, 30 each of `request/cell` and `request/gate`, 15 of `cell/gate`) *(Corrected Sep 2 (reach): 495 of 1008 under the reach-aware Bound — `step/row` 74, `step/cell` 61, `layer/row` and `matvec/row` 55 each, `layer/cell`, `matvec/cell`, `row/cell`, `row/gate` 30 each; the `request/*` and `cell/gate` counts unchanged; see §7.6)*. The decomposition $U\approx(\Lambda/c(1))\times(\text{bits per error})$ with $\Lambda/c(1)=10.7, 71.4, 214$ and bits/error $=59.6, 56.4, 54.7$ still describes `cell/gate` at $q=1/2,s=1/8$; for `request/cell` the bound is instead the RU channel, $U\approx\Lambda_q\times8192$ with $\Lambda_q=\Lambda/(-\ln(1-q))$ (54,426 predicted vs 54,744 measured at $q=1/2,\eta=10^{-2}$; 282,522 vs 284,048 at $q=1/8$), which exceeds the output once $\Lambda_q>2048$, i.e. $q\lesssim\Lambda/2048$ ($1/445$ at $\eta=10^{-2}$, $1/148$ at $10^{-6}$) — hence the vacuous 1%-overhead column. For `request/row` a single VU error in the one-hot (524 kbit declared) is charged the whole request at the VU price $c(1)$, so $U\approx(\Lambda/c(1))\times8192$: 584,990 at $q=1/2,s=1/8$ (vs 54,744 for `request/cell`), 2,396,735 at $q=1/8,s=1/8$ (vs 284,048), identical at $s=1$.

### 6.5 Is $h=1$ consistent with "16% for hashing everything"? What to say about calibrating $h$

Under $h=1$ (one hash costs one `add`), hashing every value of the 70B circuit costs $n\cdot h/\text{honest} = 0.667$ — **67% of the honest computation**, not 16%. The two numbers are not about the same thing (the outline's is "for GPT-2"; the report's is the toy ISA's cost units on a 70B shape), but they cannot both be right for the same $h$: 16% would need $h\approx0.24$ in these units (or a circuit with far fewer values per unit of cost, e.g. wide fused gates). Conversely the outline's own "1% budget" is not met by the frontier's headline policies ($0.896\times$ overhead at $q=1/2, s=1/8$); the 1%-overhead column of the calibration table exists ($q\le1/128$) and buys $U$ in the 100 kbit–3 Mbit range at $\eta=1/100$. *(Corrected Sep 2: under the recompute-honest model the 1%-overhead column needs $q\le1/512$ with request RUs and is then vacuous at $\eta\le10^{-2}$ — $\Lambda_q\times8192$ exceeds the output — and non-vacuous only at $\eta=1/2$: 2.9 Mbit (17%) at `request/cell`, $q=1/512$, $s=1/8$. The 10%-overhead column buys 1.14 Mbit (7.1%) at $\eta=10^{-2}$, `request/cell`, $q=1/32$, $s=1/8$. Fine RU levels cannot enter any column below 98%.)* Recommendations for the text:
...
2. Either derive the 16% (values per FLOP for GPT-2 under a stated hash cost) or drop it; if kept, it fixes $h$ and the frontier overheads should be restated at that $h$ (they scale as $q(1 + 0.667h + s)$ for the closed `request` RUs and as $1 + q(0.667h + s)$ for every finer RU level — corrected Sep 2).
...
4. Cite the honest-server numbers in the form "overhead $\approx q(1.67+s)$ honest-costs *for request RUs* ($\approx 1 + q(0.67+s)$ for any RU level that reads activations or the KV cache, since the honest prover must re-run the request to replay it — corrected Sep 2), verifier work $\approx 79\,qs$ with `cell` or `row` VUs and $\approx 119\,qs$ with `gate` VUs", with the caveat that all are gate counts, not measured time (`frontier-report.md` "Caveats").
...
`src/veritor/evaluation/serving.py` writes the per-kind table the compiler would produce for the toy decoder of `constructors/lm.py` at any dimensions (docstring: *"the structure is the toy's, gate for gate: `dot_k` is `k` products and a sum tree, `attend_head_c` is `c` scores, their squares, the mix and the shift, a token's layer is four projections, the heads, two residual sums and the square MLP, a request is a prefill over its prompt then one decode step per generated token, and the run is the weights unit followed by the requests"*). Replay levels: `request` (one unit per request), `step` (one unit per synchronous decode step of a batch, `ClusterG`'s layout), `layer` (one token's layer), `matvec` (one matrix-vector product or one head), `row` (one dot product / head / one-hot / argmax / block of residual or square cells), `cell` (*"every unit has one output: a dot product or a single gate, so no unit's cut exceeds a word"*). Verification levels: `layer`, `row` (the toy's marks), `cell` (one VU per dot product or single-output cell; added Sep 2, so `DEFAULT_PARTITIONS` now has 12 entries: the seven of §1.7 plus `request/cell`, `step/cell`, `layer/cell`, `matvec/cell`, `row/cell`), `gate` (every gate its own unit). Since Sep 2 every kind also carries `closed` (`KindSummary.closed`, derived from the builder's wiring: `weights`, `request`, `prefill`, `prefill_step`, `prompt` are closed; everything decode-side is not), checked against compiled `RequestsG`/`ClusterG` at toy scale in `test_serving.py::rows_of`. *"Every gate lies in exactly one unit of each level"*; source gates are always verification units; the weights are one replay unit. Validated against compiled `RequestsG` and `ClusterG` at toy scale (`test_request_row_is_the_profile_of_requests_g`, `test_step_row_is_the_profile_of_cluster_g_on_identical_requests`, `test_every_partition_tiles_the_same_gates`, `test_finer_replay_levels_have_more_units_and_the_cell_level_one_word_each`, `test_verification_levels_refine`, `test_a_frontier_sized_table_is_written_in_milliseconds`).

`src/veritor/evaluation/frontier.py`: `FRONTIER_SHAPE` (§6.4), `DEFAULT_ETAS = (1/2, 1/100, 1/10^6)`, `DEFAULT_GRID = q ∈ {1/2, 1/8, 1/32, 1/128, 1/512, 1/2048, 1/8192} × s ∈ {1, 1/8, 1/64, 1/512}`, `DEFAULT_PARTITIONS` = the seven of §1.7, `FRONTIER_OPTIONS = BoundOptions(knapsack=False, max_buckets=1<<22)` (Laplace only), `Point` (partition, $q$, $s$, $\eta$, `bits`, `out_bits`, `overhead` = Cost.total/honest, `work` = expected_work/honest, seconds, and since Sep 2 `recompute` = Cost.recompute/honest), `honest_cost`, `price`, `sweep`, `certify(points, eta, max_overhead, max_work)` (smallest `bits` within budgets; ties to cheaper prover then verifier), `save`/`load`, `calibration_table`, `partition_table`. Tests: `tests/veritor/evaluation/test_frontier.py`.
...
> **Corrected Sep 2 — the bullets of §7.5 above are superseded** by the re-run report (recompute-honest cost model, 12 partitions, 1008 points). What changes and what does not:
> - *Unchanged:* every $U$ value; the monotonicities (now 0 violations of 1008/3024/1512 pairs); "the bound is governed by the interface of the unsampled RU"; the `request/row` vs `request/gate` VU-width effect. `cell/gate ≤ request/*` at 84/84 (`row`) and 83/84 (`cell`, `gate`).
> - *Wrong and corrected:* "prover overhead is the same for every partition" and "marking at `cell/gate` costs nothing". Only closed RUs (`request`; `prefill_step`; the weights) replay at $q\times$ their cost. Every RU level that reads activations or the KV cache (`step`, `layer`, `matvec`, `row`, `cell`) forces the honest prover to re-execute the enclosing request (or, for a decode step, the whole run) with probability $1-(1-q)^m\approx1$, so `recompute = 1.000` of the honest cost at every grid policy and prover overhead is $1+q(0.667+s)$: `cell/gate` costs 1.396 at $q=1/2,s=1/8$ and never less than 1.0002.
> - *Corrected calibration* (overhead $\le1$, work $\le10$): `request/cell` at $q=1/2,s=1/8$: $U = 8{,}248$ / $54{,}744$ / $164{,}200$ bits (0.008 / 0.052 / 0.157 bits per generated token) at $\eta=1/2,10^{-2},10^{-6}$, overhead 0.896 (recompute 0.500), work 4.96. At (overhead $\le10\%$, work $\le1$): `request/cell` $q=1/32,s=1/8$: 180,026 / 1,195,659 / 3,586,234 bits (0.17 / 1.14 / 3.42 per token). At (overhead $\le1\%$, work $\le10\%$): only $\eta=1/2$ is non-vacuous (2,908,248 bits, `request/cell` $q=1/512,s=1/8$). `cell/gate` appears in the calibration tables only from 200% prover overhead on, where it is 1.1× (1% work) to 140× (10000% work) below `request/cell`.
> - *New observation:* with request RUs the VU level decides the bound once $s<1$. `request/row` charges a single failed one-hot VU (524 kbit declared) the whole request at the VU price $c(1)$, giving $U\approx(\Lambda/c(1))\times8192$; `request/cell` (one VU per dot product, 16 bits) leaves the RU channel $\Lambda_q\times8192$ as the bound. At $q=1/8,s=1/8,\eta=10^{-2}$: 2,396,735 vs 284,048 bits at identical overhead 0.224 and work 1.24; at $s=1$ they coincide (282,809 vs 282,775). `request/gate` equals `request/cell` in $U$ at 1.5× the verifier work (118.7 vs 79.3 $qs$). **"Request RUs, dot-product VUs" is the honest server's natural operating point**; the paper's honest server should be `request/cell`, not `cell/gate`.
> - *Cap and reach:* 802 of 1008 points are at the cap, all 672 points of the eight `step`/`layer`/`matvec`/`row` partitions among them. Part of that is a cap in `Bound` rather than in the theorem: `Bound` charges an unsampled node $\min(\text{interface},\ \text{whole output})$ whereas the theorem's cut is $\min(\text{interface},\ \text{reachable output})$ (§7.6). A reach-aware `Bound` is being implemented separately and the report will be re-run under it; the cost rows of those partitions will not change. *(Corrected Sep 2 (reach): done, commit fb3d074, report re-run; see the note under §7.6. Points at the cap fell from 802 to 495; costs unchanged; no operating point changed.)*

### 7.6 The reach-aware refinement of Bound (implemented Sep 2, commit fb3d074)

> **Corrected Sep 2 (reach).** The paragraph below was written while this was pending; it is now implemented and the frontier (`docs/frontier-report.md`, 1008 points) has been re-run under it. What landed: `KindSummary.reach_bits` is an upper bound, over the copies of a kind, on the width of the circuit outputs reachable from a copy along argument reads, computed in `Index.kinds()` top-down over the definition DAG at step granularity (within a definition, a step depends on every earlier step whose slots any of its argument runs meet; a call or `repeat` is one node; `Down(j)` is the transitive closure; a child called at step $j$ receives what $j$ reaches and keeps the maximum over call sites; at the root the declared outputs are the circuit outputs weighed per step segment, one copy of a `repeat` charged at most one copy's share; below the root every output of a copy shares the copy's reach). `Bound` charges every node $\kappa=\min(\mathrm{out\_bits},\mathrm{reach\_bits})$ (`bound.cut_bits`); the root's cap stays. **Theorem side:** the outputs reachable from $S$ are a downstream cut for $S$ (every path from $S$ to the output ends at one of them), exactly as $\mathrm{Out}(S)$ is (every path out of $S$ leaves through a declared output), so this is Corollary 5.7 / §5.4 with a second cut and needs no new theorem; any superset of the reachable outputs is also a cut, so the structural over-approximation is sound. `KindTable` checks $0\le\mathrm{reach\_bits}\le$ root `out_bits`; the analysis reference charges the same min with the exact per-node reach; a brute-force oracle over the flat circuit (`tests/veritor/core/test_reach.py`) checks every table for soundness and, for the toy layouts, the wave result is exact. The serving builder sets `reach_bits` from the run's dataflow: a `request` reaches its own 8,192 token bits (requests never read each other); the steps of one wave of 32 requests are chained through the tokens and the cache, so a `prefill_step` reaches the wave's $32\times512\times16=262{,}144$ bits and the decode step at context $c$ the wave's $32\times(1024-c)\times16$ remaining bits; the next wave's prefill reads only the weights; the weights reach everything.
>
> **Headline comparisons** (before = report of commit ebf31f0, after = fb3d074; costs, recompute and work identical at all 1008 points; `bits` never increased, fell at 307 points; at the cap 802 → 495). At $q=1/8,s=1/8,\eta=10^{-2}$: `request/row` 2,396,735 / `request/cell` 284,048 / `request/gate` 284,053 / `cell/gate` 15,864 unchanged; `layer/row` and `matvec/row` 16,777,216 → 2,399,690; `layer/cell` → 284,580, `matvec/cell` → 284,714, `row/cell` → 284,638, `row/gate` → 284,640 (all within 0.25% of `request/cell`: every RU inside a request now reaches exactly that request's 8,192 bits whatever its interface, so their RU channel is $\Lambda_q\times8192$ plus a slightly larger position term); `step/cell` 16,777,216 → 9,081,475 (54%); `step/row` still capped at that policy. The step result: under FCFS with identical requests the run is 64 waves of 32 requests, a step reaches the remaining tokens of its own wave ($\le262{,}144$ bits), so a free step leaks up to 32× a free request and the `step` RU channel is $\Lambda_q\times262{,}144$ (9,040,704 predicted vs 9,040,767 measured for `step/cell` at $q=1/8,s=1$; 1,741,647 vs 1,741,678 at $q=1/2$), non-vacuous only where $\Lambda_q<64$ ($q\ge1/8$ at $\eta=10^{-2}$, $q=1/2$ at $10^{-6}$). The estimates in the paragraph below ("roughly $6\times262$ kbit $\approx1.6$ Mbit" for `step/row` at $q=1/2$; "$\approx50$ kbit" for `layer/row`, `matvec/row`) were right in order of magnitude for `cell` VUs (`step/cell` 1,749,765 at $q=1/2,s=1/8$; `layer/cell` 54,846, `matvec/cell` 54,872) but not for `row` VUs, where the one-hot VU is charged the enclosing RU's whole reach at the VU price (`step/row` 9,685,509; `layer/row` = `matvec/row` 585,705 at $q=1/2,s=1/8$). **No operating point changed:** under (overhead $\le1$, work $\le10$) at $\eta=10^{-2}$ the answer is still `request/cell` $q=1/2,s=1/8$, $U=54{,}744$; `step/*` and `layer/*` admit only $q=1/8192$ (capped); `matvec/*`, `row/*`, `cell/gate` admit nothing under 100% overhead — reach makes the intermediate RU levels *finite* but their recompute is still 100%. **Residual looseness** (documented in `index.py`): reach is at step granularity and below the root a definition's output pieces are not told apart — a decode step inside a request is charged the whole request's tokens rather than its remaining ones, and the one-hot of one slot inside a step is charged the whole wave's 262,144 bits; per-slot/per-piece tracking is a possible refinement. For D-9: write "implemented" — Bound uses, per node, the smaller of its declared interface and the width of the outputs downstream of it, both downstream cuts in the sense of §5.4.

Status at the time of writing (superseded above): **not implemented**; theorem drafted in `docs/overnight-report.md` §4 item 4 and §5.1: *"with errors confined to $S_1\cup\cdots\cup S_m$, $|\text{reachable outputs}|\le\prod_j 2^{\min(\mathrm{out\_bits}(S_j),\ \mathrm{reach\_bits}(S_j))}$ where $\mathrm{reach\_bits}(S)$ is the width of the circuit outputs downstream of $S$. Proof: fix all cuts but $c_j$; varying $c_j$ changes outputs only inside $\mathrm{reach}(S_j)$ and through at most $2^{\mathrm{out\_bits}(S_j)}$ cut values, so the image grows by at most $2^{\min(\cdot,\cdot)}$ per node; induct on $j$."* This is Corollary 5.7 applied with the cut $\mathrm{reach}(S)\subseteq\mathrm{out}(C)$ itself (the outputs downstream of $S$ form a downstream cut for $S$), so it needs no new theorem in §5 — only the observation that $\kappa(A)\le W(\text{outputs reachable from }A)$ and its use per node in the fold. Implementation plan (report §5.1): a reverse-dependency pass over the hierarchy at $O(|\text{description}|)$, per-kind `reach_bits` as the max over copies (sound), fold with $\min(\mathrm{out\_bits},\mathrm{reach\_bits})$; validate against the BFS oracle on the cluster fixtures. Measured effect on the toy cluster (56,600 gates, 41 replay units, 2,032 output bits, $\eta=10^{-2}$): $q=1/2,s=1$: cap 2,032 → 869; $q=1/2,s=1/2$: 2,032 → 1,152; $q=1/4,s=1/2$: still capped.
...
Suggested: $U$ (log scale, bits or % of output) against $qs$ (log scale) for each partition at $\eta=1/100$, with verifier work $=118.7\,qs$ as a secondary axis; `cell/gate`, `request/gate`, `request/row` as three curves, the intermediate four as a flat line at the cap (labelled "not reach-aware") *(Corrected Sep 2 (reach): no longer flat — with the reach-aware Bound `layer/cell`, `matvec/cell`, `row/cell` track `request/cell` to within 0.25% and `step/cell` sits 32× above it; the curves to draw are `request/cell`, `request/row`, `step/cell`, `cell/gate`, with the intermediate `*/cell` levels coinciding with `request/cell`)*. A second panel or table: the calibration table (work × overhead → $U_{\max}$) for $\eta=1/100$. Data: `docs/data/frontier-70b.json` via `veritor.evaluation.frontier.load`; `calibration_table`, `partition_table` render the tables.
...
| downstream cut / $\kappa$ / reach | `out_bits` per node (interface cut); `KindSummary.reach_bits` (reach cut, since fb3d074); `bound.cut_bits` = their min; `reference.cut_bits`, `cover_bits` (small-circuit oracles); `core/index.py::output_reach` | `analysis/reference.py`, `core/index.py` |
...
17. Say what is pending: SP1 calibration of $c_{\mathrm{proof}}$ and $h$; the 16% figure; value semantics. *(Corrected Sep 2 (reach): the reach-aware Bound is no longer pending — commit fb3d074; what remains pending on that front is per-slot/per-piece reach below the root.)*
