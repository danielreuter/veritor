...
> - *Cap and reach:* 802 of 1008 points are at the cap, all 672 points of the eight `step`/`layer`/`matvec`/`row` partitions among them. Part of that is a cap in `Bound` rather than in the theorem: `Bound` charges an unsampled node $\min(\text{interface},\ \text{whole output})$ whereas the theorem's cut is $\min(\text{interface},\ \text{reachable output})$ (§7.6). A reach-aware `Bound` is being implemented separately and the report will be re-run under it; the cost rows of those partitions will not change.
