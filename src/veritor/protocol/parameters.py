"""What the verifier fixes on its own: ``eta``, ``U_max`` and ``W_max``.

The client proposes the sampling rates ``theta = (q, s)``.  The acceptance
threshold ``eta`` defines the security statement, so it is never taken from
the client; ``U_max`` bounds the capacity ``U = Bound(C, I, theta)`` the
verifier will underwrite; ``W_max`` bounds the verifier's own expected work,
priced by :func:`expected_work` from counts alone before any commitment is
accepted.
"""
