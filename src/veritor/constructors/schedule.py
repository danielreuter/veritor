...
its prefill by a later join (on whichever pod is free).  Every attempt
...
for a request are the first ``max(length)`` positions of its decoding, each
produced by exactly one attempt.
"""
...
_MAGIC = b"veritor/schedule/v2\0"
_WORD = 4
_JOIN_WORDS = 5
...
class Join:
    ...
    The join step is the prefill (the request's first token); each of the
    ``length - 1`` following steps is a decode step generating one token.
    """
...
class Occupant:
    ...
    request: int
    generated: int
    """Tokens generated by this attempt before this step: ``0`` at the join (prefill) step."""
    join: int
...
class Schedule:
    ...
    the requests: every request joins at least once, no attempt is longer
    than ``max_new``, and a request's attempts never overlap in time -- a
    restart begins no earlier than the step at which the attempt it replaces
    was cut off.  Requests whose generation is cut short by a failure, an
    early stop or the end of the run simply produce fewer tokens; every such
    choice is the client's and is paid for by the encoded length of the
    schedule.
    """
    ...
    def encode(self) -> bytes:
        ...
        A join is ``pod | step | slot | request | length``.
        """
        ...
        for join in self.joins:
            for value in (join.pod, join.step, join.slot, join.request, join.length):
                out.extend(value.to_bytes(_WORD, "big"))
        return bytes(out)
    ...
    def decode(cls, data: bytes) -> Schedule:
        ...
        joins = tuple(
            Join(*(int.from_bytes(rest[i + k * _WORD : i + (k + 1) * _WORD], "big") for k in range(_JOIN_WORDS)))
    ...
    def attempts(self, requests: Sequence[Request]) -> dict[int, tuple[int, ...]]:
        ...
        Every join names an existing request and is no longer than its
        ``max_new``; every request joins at least once; and a request's
        attempts do not overlap in time.
        """
        ...
        for index, join in enumerate(self.joins):
            ...
            if join.length > requests[join.request].max_new:
                raise ScheduleError(f"request {join.request} is scheduled for more than max_new tokens")
            by_request.setdefault(join.request, []).append(index)
        if len(by_request) != len(requests):
            missing = sorted(set(range(len(requests))) - set(by_request))
            raise ScheduleError(f"requests never scheduled: {missing}")
        result: dict[int, tuple[int, ...]] = {}
        for request, indices in by_request.items():
            indices.sort(key=lambda index: self.joins[index].step)
    ...
    def streamed_before(self, requests: Sequence[Request]) -> tuple[int, ...]:
        ...
        An attempt recomputes its request from position ``0``; only the
        positions from ``streamed_before`` up to its ``length`` are new
        outputs.  ``0`` for a request's first attempt.
        """

        before = [0] * len(self.joins)
        for indices in self.attempts(requests).values():
            streamed = 0
            for index in indices:
                before[index] = streamed
                streamed = max(streamed, self.joins[index].length)
        return tuple(before)

    def active_steps(self, requests: Sequence[Request]) -> dict[int, int]:
        """Tokens each request streams: the longest of its attempts."""

        return {
            request: max(self.joins[index].length for index in indices)
            for request, indices in self.attempts(requests).items()
        }
        ...
        for index, join in enumerate(self.joins):
            for offset in range(join.length):
                table.setdefault((join.pod, join.step + offset), []).append(
                    Occupant(join.slot, join.request, offset, index)
                )
