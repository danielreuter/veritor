...
def test_theta_with_an_enormous_denominator_is_resource_limit(model, sec):
    ...
    eta = VerifierParameters(Fraction(1, (1 << 65) + 1))
...
def test_run_whose_expected_work_exceeds_w_max_is_work_budget_exceeded(model, sec):
    ...
    below = VerifierParameters(max_work=math.floor(work))
    ...
    assert run.transcript is None and run.report.sampled_replay_units == ()
    above = VerifierParameters(max_work=math.ceil(work))
...
def test_eta_is_the_verifiers_and_bound_into_the_header(model, sec):
    parameters = VerifierParameters(Fraction(1, 8))
    ...
    with pytest.raises(TypeError):
        VerifierParameters(0.25)  # type: ignore[arg-type]
    ...
    other = model.expectation(sec.HALVES, parameters=VerifierParameters(Fraction(1, 16)))
...
def test_default_parameters_waive_u_max_and_admit_a_policy_that_checks_nothing(model, sec):
    """GAP: ``VerifierParameters()`` has ``U_max = None``; ``theta = (0, 0)`` is then admitted.
