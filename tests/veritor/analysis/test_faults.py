...
def vu_count(compiled) -> int:
    return sum(row.copies for row in compiled.kind_table().rows if row.role == VERIFICATION)
...
def test_fault_allowance_is_linear_and_validated() -> None:
    ...
    with pytest.raises(ValueError):
        declared_bits(compiled.kind_table(), VerificationPolicy(1, 1), ETA, BoundOptions(), -1, 0.0)


def test_full_verification_pays_exactly_the_allowance_until_the_interface_caps_it() -> None:
    ...
    for f in range(1, 4):  # q = 1: the q-challenge reveals nothing, so a declaration is one VU
...
@pytest.mark.parametrize(
    ...
    [
        ...
        VerificationPolicy(Fraction(1, 2), Fraction(1, 2)),  # the q-challenge is informative
        VerificationPolicy(Fraction(1, 2), 1),  # ... and every sampled VU is checked
    ],
)
def test_declared_capacity_is_the_smallest_rigorous_bound(policy: VerificationPolicy) -> None:
    compiled = build_compiled((4, 4, 4, 4))
    ...
    for f in (1, 2):
        ...
        expected = min(min(rigorous_candidates(compiled, policy, LOOSE, f)), float(base.out_bits))
...
def test_declaring_after_the_q_challenge_costs_more_than_a_fixed_declaration() -> None:
    ...
    assert declared_bits(compiled.kind_table(), policy, LOOSE, BoundOptions(), 0, 2.5) == 2.5
