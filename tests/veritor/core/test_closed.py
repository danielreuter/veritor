...
def closed_by_key(
    tracer: Tracer, root: TracedDefinition, **named: TracedDefinition
) -> dict[str, bool]:
    """``closed`` of the named definitions (and ``root``) in the index of ``root``."""

    kinds = {
        row.kind: row
        for row in Index(parse_description(tracer.serialize(root), GATES).root).kinds()
    }
    named["root"] = root
...
def test_a_kind_fed_weights_only_is_closed_and_so_are_the_root_and_the_source_cells() -> (
    None
):
    tracer = Tracer(GATES)
    plus, _ = cells(tracer)
    ...
    flags = closed_by_key(
        ...
        root,
        plus=plus,
        w=tracer.source_cell("weight"),
        x=tracer.source_cell("input"),
    )
...
def test_a_kind_fed_a_computed_value_is_not_closed() -> None:
    ...
    assert closed_by_key(tracer, root, plus=plus, times=times) == {
        "plus": False,
        "times": True,
        "root": True,
    }
...
def test_a_kind_fed_a_callers_port_is_closed_iff_that_port_is_retained() -> None:
    ...
    def outer(v):
        return [
            plus(v[1], v[2]),
            times(v[0], v[1]),
        ]  # ``plus`` reads ports 1, 2; ``times`` ports 0, 1
...
def test_a_repeat_reading_another_repeats_outputs_across_copies_is_not_closed() -> None:
    ...
    def root(_v):
        ...
        products = tracer.repeat(
            n, times, w[0].by(1), w[1].by(1)
        )  # copy j: w[j] * w[j + 1], closed
        return tracer.repeat(
            n, plus, products[0].by(1), w[0].by(1)
        )  # copy j: products[j] + w[j]
...
def test_a_repeat_whose_strided_argument_crosses_from_weights_into_computed_values_is_not_closed() -> (
    None
):
    ...
    def root(_v):
        w = tracer.weights(4)
        tracer.repeat(
            2, times, w[0].by(1), w[1].by(1)
        )  # two products in slots 4, 5 after the four weights
...
def test_a_repeat_over_a_callers_ports_is_closed_iff_the_ports_it_strides_over_are_retained() -> (
    None
):
    ...
    def root(_v):
        ...
        return outer(
            w[0], w[1], product, w[3]
        )  # port 2 of ``outer`` is transient: copy 1 of the repeat
    ...
    assert closed_by_key(tracer, root2, outer=outer2, plus=plus) == {
        "outer": True,
        "plus": True,
        "root": True,
    }
...
def test_the_table_records_the_flag_and_requires_portless_kinds_to_be_closed() -> None:
    ...
    rows = tuple(
        replace(row, closed=False) if row.input_count == 0 else row
