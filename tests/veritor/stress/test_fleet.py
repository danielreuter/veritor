...
def per_request(
    model: Model, requests: tuple[Request, ...], tensor_parallel: int = 1
) -> Served:
...
def test_n2_tensor_parallelism(scenario: Recorder, model: Model) -> None:
    ...
    verification = {
        degree: set(run.kinds(VERIFICATION)) for degree, run in runs.items()
    }
    replay = {degree: set(run.kinds(REPLAY)) for degree, run in runs.items()}
    assert (
        verification[1].isdisjoint(verification[2] - verification[1])
        and verification[2] != verification[4]
    )
    assert (
        len(set.intersection(*replay.values())) == 1
    )  # only the weights unit is shared: the request kinds differ
    ...
    assert (
        len(
            {
                (result.bits, round(result.knapsack_bits, 6))
                for result in bounds.values()
            }
        )
        == 1
    )
    ...
    dots = {
        degree: RequestsG(model.shape, tensor_parallel=degree).lm.dot(
            model.shape.hidden
        )
        for degree in (1, 2, 4)
    }
    sizes = {
        degree: runs[degree].measurement.compiled.kind_table() for degree in (1, 2, 4)
    }
    gates = {
        degree: next(
            row.size for row in sizes[degree].rows if row.kind == dots[degree].digest
        )
        for degree in (1, 2, 4)
    }
    ...
    for degree, letter in ((1, "a"), (2, "b"), (4, "c")):
        ...
        scenario.record(
            ...
            notes=run.notes(
                "the TP degree is public configuration in the constructor's manifest, not advice"
            ),
        )
...
def test_n3_heterogeneous_fleet(scenario: Recorder, model: Model) -> None:
    ...
    assert (
        fleet.gate_set.id == "veritor.toy-isa-fleet@1"
        and len(fleet.gate_set) == 2 * 6 + 2
    )
    ...
    steps_mixed = {kind for kind, _ in mixed.kinds(REPLAY).items()} - {
        fleet.lm.weights_unit().digest
    }
    steps_same = {kind for kind, _ in same.kinds(REPLAY).items()} - {
        homogeneous.lm.weights_unit().digest
    }
    ...
    assert (
        sm80.dot(model.shape.d_model).digest in verification
        and sm90.dot(model.shape.d_model).digest in verification
    )
    assert (
        sm80.argmax().digest in verification and sm90.argmax().digest in verification
    )  # both heads decide
    ...
    with pytest.raises(CompileError, match="unknown gate"):
        Compiler(model.gate_set).compile(
            mixed.measurement.description, mixed.measurement.compilation.inputs
        )
    with pytest.raises(CompileError, match="unknown gate"):
        Compiler(fleet.gate_set).compile(
            same.measurement.description, same.measurement.compilation.inputs
        )
