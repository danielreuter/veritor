...
def cluster(model: Model, requests: tuple[Request, ...], schedule: Schedule) -> Served:
    ...
    return serve(constructor, requests, schedule.encode(), model.gate_set, model.weights, layout, len(requests))
...
def reordered_fcfs(requests: tuple[Request, ...], order: tuple[int, ...], pods: int, slots: int, steps: int) -> Schedule:
    """FCFS over the requests admitted in ``order``, joins naming the original indices."""

    schedule = schedule_fcfs(tuple(requests[index] for index in order), pods, slots, steps)
    joins = tuple(
        Join(join.pod, join.step, join.slot, order[join.request], join.length, join.resume, join.chunk)
        for join in schedule.joins
    )
    return Schedule(pods, slots, steps, tuple(sorted(joins)))


def request_kinds(constructor: RequestsG, requests: tuple[Request, ...]) -> tuple[str, ...]:
    """The digest of each request's kind, request by request."""

    return tuple(constructor.request(*constructor.kind_of(request)).digest for request in requests)


# -- S1: continual batching ---------------------------------------------------------


def test_s1_continual_batching_under_three_schedules(scenario: Recorder, model: Model) -> None:
    reference = reference_generate(model.shape, model.parameters, REQUESTS)
    schedules = {
        "fcfs": schedule_fcfs(REQUESTS, PODS, SLOTS, STEPS),
        "reversed admission": reordered_fcfs(REQUESTS, (4, 3, 2, 1, 0), PODS, SLOTS, STEPS),
        "one slot per pod": Schedule(PODS, SLOTS, STEPS, schedule_fcfs(REQUESTS, PODS, 1, STEPS).joins),
    }
    served = {name: cluster(model, REQUESTS, schedule) for name, schedule in schedules.items()}
    requests = per_request(model, REQUESTS)

    # identical outputs under every schedule and with request units
    assert all(run.tokens == reference for run in served.values()) and requests.tokens == reference
    ...
    assert all(run.advice_bits == 8 * len(schedule.encode()) for run, schedule in zip(served.values(), schedules.values(), strict=True))
    ...
    for order in ((0, 1, 2, 3, 4), (4, 3, 2, 1, 0), (2, 0, 4, 1, 3)):
        ...
        assert request_kinds(RequestsG(model.shape), admitted) == tuple(kinds[index] for index in order)
    for run in served.values():
        assert set(kinds) <= set(requests.kinds(REPLAY)) and set(kinds).isdisjoint(run.kinds(REPLAY))
    ...
    for letter, (name, schedule) in zip("abc", schedules.items(), strict=True):
        ...
        scenario.record(
            ...
            notes=run.notes(f"{len(run.kinds(REPLAY))} RU kinds; description digest differs per schedule"),
        )
    scenario.record(
        id="S1d",
        ...
        notes=requests.notes("the schedule is the server's business: not in x, not in a, not in the circuit"),
    )
...
def test_s3_preemption_by_recompute(scenario: Recorder, model: Model) -> None:
    ...
    evicted = Schedule(
        ...
        tuple(sorted((
            *(join for join in plain.joins if join.request != 1),
            Join(0, 0, 1, 1, 2),  # the first attempt: 2 tokens, then the eviction
            Join(1, 5, 1, 1, 4),  # prefilled again from scratch on pod 1: positions 0, 1 recomputed, 2, 3 streamed
        ))),
    )
    ...
    assert after.digest != before.digest and after.price.honest > before.price.honest  # the recompute is in the circuit
    second = next(index for index, join in enumerate(evicted.joins) if join.request == 1 and join.step == 5)
    assert evicted.streamed_before(REQUESTS)[second] == 2  # positions 0 and 1 are recomputed, not streamed again
    ...
    scenario.record(
        ...
        notes=after.notes(f"honest replay cost {after.price.honest} vs {before.price.honest} without the eviction"),
    )
    scenario.record(
        id="S3b",
...
def test_s4_preemption_by_swap(scenario: Recorder, model: Model) -> None:
    ...
    swapped = Schedule(
        ...
        tuple(sorted((
            *(join for join in plain.joins if join.request != 1),
            Join(0, 0, 1, 1, 2),  # prefill and one decode, then the cache is swapped out
            Join(0, 2 + gap, 1, 1, 2, resume=True),  # resumes: reads the KV declared ``gap`` steps earlier
        ))),
    )
    ...
    scenario.record(
        ...
        notes=after.notes("Join.resume marks the attempt; the gap is wiring, not a kind: decode_c is the same kind resumed or not"),
    )
    scenario.record(
        id="S4b",
...
def test_s5_chunked_prefill(scenario: Recorder, model: Model) -> None:
    ...
    assert after.tokens == reference == before.tokens == requests.tokens
    assert after.digest != before.digest  # step units: chunk_3 and prefill over 6 cached positions are new kinds
    ...
    scenario.record(
        ...
        notes=after.notes("Join.chunk carries the chunk size; each chunk declares its KV rows for the next step"),
    )
    scenario.record(
        id="S5b",
...
def test_s6_prefill_decode_disaggregation(scenario: Recorder, model: Model) -> None:
    ...
    for index, request in enumerate(requests_):
        joins.append(Join(0, index, 0, index, 1))  # prefill only, on pod 0, one request per step
        if request.max_new > 1:
            joins.append(Join(1, index + 1, index, index, request.max_new - 1, resume=True))  # decode on pod 1
    disaggregated = Schedule(2, 3, 6, tuple(sorted(joins)))
    colocated = schedule_fcfs(requests_, 2, 3, 6)
    apart, together = cluster(model, requests_, disaggregated), cluster(model, requests_, colocated)
    ...
    scenario.record(
        ...
        notes=apart.notes("the cluster is synchronous and time-major: any step may read what any earlier step of any pod declared"),
    )
    scenario.record(
        id="S6b",
...
def test_s8_a_discarded_duplicate_is_not_in_the_circuit(scenario: Recorder, model: Model) -> None:
    ...
    plain = schedule_fcfs(REQUESTS, PODS, SLOTS, STEPS)
    declared = Schedule(
        PODS, SLOTS, STEPS, tuple(sorted((*plain.joins, Join(1, 6, 1, 2, 2))))  # a second full attempt of request 2
    )
    requests, again = per_request(model, REQUESTS), per_request(model, REQUESTS)
    without, with_duplicate = cluster(model, REQUESTS, plain), cluster(model, REQUESTS, declared)

    assert requests.digest == again.digest and requests.tokens == reference
    assert without.tokens == with_duplicate.tokens == reference
    assert with_duplicate.price.honest > without.price.honest  # the declared duplicate is replayed like any step
    assert with_duplicate.outputs == without.outputs  # its tokens are declared, not output
    ...
    scenario.record(
        ...
        notes=requests.notes("what is not in (x, a) is not in C: a discarded execution has no gates"),
    )
    scenario.record(
        id="S8b",
        ...
        notes=with_duplicate.notes(f"honest replay cost {with_duplicate.price.honest} vs {without.price.honest} when the schedule omits it"),
    )
