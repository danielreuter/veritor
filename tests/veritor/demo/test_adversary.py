...
class Run:
    def __init__(self) -> None:
        ...
        self.compilation = Compile(
            constructor, self.simulation.requests, advice, GATE_SET, max_advice_bits=8 * len(advice)
        )
        self.compiled = self.compilation.compiled
        self.layout = constructor.output_layout(self.simulation.requests, self.simulation.schedule)
        ...
        assert self.outputs == tuple(t for response in self.simulation.streamed for t in response)
...
def test_carriers_are_the_last_tokens_round_robin_over_requests():
    ...
    assert carriers(layout, 4) == (1, 2, 3, 5)  # then the second to last of the first request
...
def test_the_secret_is_written_into_the_head_vus_and_read_back(run):
    ...
    attack = plan_attack(
        run.compiled, run.compilation.inputs, run.weights, run.layout, secret, SHAPE.vocab_bits
    )
    assert attack.bits == 12 and len(attack.carriers) == 4
    assert attack.addresses == tuple(run.compiled.circuit.outputs[c] for c in attack.carriers)
    ...
    for address, carrier in zip(attack.addresses, attack.carriers, strict=True):
        assert (address in attack.corrupted) == (attack.outputs[carrier] != run.outputs[carrier])
    ...
    wrong = [
        ...
        and not circuit.check_gate(
            address, tuple(attack.values[a] for a in circuit[address].args), attack.values[address]
        )
    ]
    assert wrong == sorted(attack.corrupted)
    with pytest.raises(ValueError):
        plan_attack(
            run.compiled, run.compilation.inputs, run.weights, run.layout, "01", SHAPE.vocab_bits
        )
...
def test_the_predicted_survival_is_the_product_over_replay_units(run):
    ...
    one = plan_attack(run.compiled, run.compilation.inputs, run.weights, run.layout, chunk, 3)
    assert len(one.corrupted) == 1 and predicted_survival(policy, one) == 1 - policy.q * policy.s
    same = plan_attack(run.compiled, run.compilation.inputs, run.weights, run.layout, honest, 3)
...
def test_the_extreme_policies_detect_everything_or_nothing(run):
    ...
    attack = plan_attack(run.compiled, run.compilation.inputs, run.weights, run.layout, chunk, 3)
    ...
    caught = protocol_trials(
        run.compilation, VerificationPolicy(1, 1), parameters, run.kappa, run.tree, attack, 1
    )
    ...
    escaped = protocol_trials(
        run.compilation, VerificationPolicy(0, 1), parameters, run.kappa, run.tree, attack, 1
    )
...
def test_the_selection_law_tracks_the_prediction_over_many_seeds(run):
    ...
    attack = plan_attack(run.compiled, run.compilation.inputs, run.weights, run.layout, secret, 3)
    trials = 600
    expected = float(predicted_survival(policy, attack))
    observed = survival_trials(run.compiled, policy, attack, trials, label="law") / trials
