...
def generated(constructor, compiled: Compiled, requests, parameters) -> tuple[tuple[int, ...], ...]:
...
def test_the_shared_prefix_is_the_longest_common_prefix_leaving_every_prompt_a_token() -> None:
    assert shared_prefix(REQUESTS) == 3
    assert shared_prefix((Request((1, 2, 3), 1), Request((1, 2, 3, 4), 1))) == 2  # the shortest keeps one
...
def test_it_generates_what_the_reference_and_requests_g_generate() -> None:
    ...
    assert generated(plain, compile_with(plain, REQUESTS), REQUESTS, parameters) == reference
    assert prefix.flatten_inputs(REQUESTS) == (3, 1, 4, 1, 6, 5, 2, 7, 7)  # the prefix once, then the suffixes
...
def test_the_prefix_unit_declares_its_kv_blocks_and_the_suffixes_read_them() -> None:
    ...
    assert prefix_row.input_count == SHAPE.weight_count and prefix_row.source_inputs == 3
    suffixes = [row for row in units.values() if row.kind not in (prefix_row.kind, prefix.lm.weights_unit().digest)]
    assert sorted((row.copies, row.out_count) for row in suffixes) == [(1, 1), (1, 3), (2, 2)]
...
def test_it_checks_its_requests() -> None:
    ...
    with pytest.raises(TracerError, match="share at least one token"):
        constructor((Request((1, 2), 1), Request((1,), 1)), b"")  # a prompt equal to the prefix
