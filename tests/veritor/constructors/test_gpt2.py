...
def by_name(constructor: GPT2G, table: KindTable) -> dict[str, tuple[int, int]]:
    ...
    return {
        names[row.kind]: (row.copies, row.size)
...
def components(
    constructor: GPT2G, table: KindTable, requests: tuple[Request, ...]
) -> dict[str, int]:
    ...
    return {
        ...
        "constants": 2
        * forwards,  # the two widened scalars per forward, ``widen_cell`` copies outside ``embed``
        "layer_norm": gates_of(kinds, "layer_norm"),
        "attention": gates_of(
            kinds,
            f"matvec({d},{d},True,True,False)",
            f"matvec({d},{d},True,False,False)",
        )
        + gates_with_prefix(kinds, "attend_head("),
        "mlp": gates_of(
            kinds,
            f"matvec({d},{f},True,False,False)",
            f"matvec({f},{d},True,False,False)",
            "gelu_cell",
        ),
        "residual": gates_of(kinds, "add_cell")
        - positions_of(requests) * d,  # minus the embedding adds
        "lm_head": gates_of(kinds, f"matvec({d},{vocab},False,False,True)"),
        "argmax": gates_with_prefix(kinds, "argmax_block(")
        + gates_with_prefix(kinds, "argmax_top("),
    }
...
def test_the_shape_validates_and_small_is_gpt2_small() -> None:
    ...
    assert (small.width, small.acc_width, small.d_head, small.argmax_block) == (
        16,
        32,
        64,
        64,
    ...
    assert (
        dict(small.layout())["wte"] == 50257 * 768
        and dict(small.layout())["layer11.w_proj"] == 3072 * 768
    )
    assert small.vocab_padded == 50272 and small.scalar_words() == {
        "n": 0x4440,
        "scale": 0x3E00,
        "zero": 0,
    }
    ...
    with pytest.raises(ValueError, match=f"multiples of {STEP}"):
        GPT2Shape(
            layers=1, d_model=32, heads=4, d_ff=16, vocab=5, context=4
        )  # d_head 8
    ...
    with pytest.raises(ValueError, match="not exact in BF16"):
        GPT2Shape(
            layers=1, d_model=16 * 257, heads=1, d_ff=16, vocab=5, context=4
        )  # 4112 = 2^12 (1 + 2^-8)
...
def test_the_flat_weight_layout_is_documented_and_consistent() -> None:
    ...
    assert TINY.layer_weights == 2 * d + 3 * (d * d + d) + (d * d + d) + 2 * d + (
        d * f + f
    ) + (f * d + d)
    assert TINY.state_size(3) == 2 * 2 * 3 * d
    assert TINY.scalar_words() == {
        "n": 0x4200,
        "scale": 0x3E00,
        "zero": 0,
    }  # 32.0, 0.125, 0.0
...
def test_the_closed_forms_account_for_every_computed_gate(run) -> None:
    ...
    assert budget["total"] == sum(
        gate_budget(TINY, len(r.prompt), r.max_new)["total"] for r in REQUESTS
    )
...
def test_dot_products_are_tensor_core_chains_of_k_over_16_steps(run) -> None:
    ...
    positions, predictions, keys = (
        positions_of(REQUESTS),
        predictions_of(REQUESTS),
        keys_of(REQUESTS),
    )

    # (copies, gates): the widened bias, k/16 steps, the rounding
    assert kinds[f"dot({d},True,True)"] == (
        3 * d * positions * layers,
        1 + d // STEP + 1,
    )  # q, k, v
    assert kinds[f"dot({d},True,False)"] == (
        (d + f) * positions * layers,
        1 + d // STEP,
    )  # o and fc (length d)
    assert kinds[f"dot({f},True,False)"] == (
        d * positions * layers,
        1 + f // STEP,
    )  # proj (length d_ff)
    assert kinds[f"dot({dh},False,False)"] == (
        layers * heads * keys + positions * d,
        dh // STEP,
    )  # scores + embedding
    assert kinds[f"dot({dh},False,True)"] == (
        layers * heads * dh * positions,
        1 + 1,
    )  # the mix: ceil(c/16) = 1 step
    assert kinds[f"dot({d},False,False)"] == (
        predictions * TINY.vocab,
        d // STEP,
    )  # the tied head
    inner_products = sum(
        copies for name, (copies, _) in kinds.items() if name.startswith("dot(")
    )
    assert inner_products == positions * layers * (
        4 * d + f + d
    ) + layers * heads * keys + layers * positions * d + (
    ...
    for name, (copies, size) in kinds.items():
        if name.startswith("dot("):
            ...
            assert size == int(k) // STEP + int(biased == "True") + int(
                rounded == "True"
            )
    padded_macs = STEP * steps
    padding = (
        layers * heads * dh * (positions * padded(1) - keys)
    )  # the mix pads each context to 16 keys
    assert padded_macs == macs + padding
    small = (
        GPT2Shape.small()
    )  # d_ff = 4 d: the familiar 12 d^2 MACs per layer and token
    assert (
        4 * small.d_model**2 + 2 * small.d_model * small.d_ff == 12 * small.d_model**2
    )
...
def test_the_components_match_the_closed_forms(run) -> None:
    ...
    assert kinds["ln_var"] == (
        normalised,
        2 * d + 1,
    )  # d squares, d - 1 adds, the division, ln_rstd
    assert kinds["sub_cell"] == (normalised * d, 1) and kinds["ln_out"] == (
        normalised * d,
        6,
    )
    assert kinds["gelu_cell"] == (TINY.layers * positions_of(REQUESTS) * TINY.d_ff, 2)
    assert kinds["argmax_block(11)"] == (
        predictions_of(REQUESTS),
        2 * (TINY.vocab - 1),
    )  # one block covers the vocab
    assert kinds["eq_cell"] == (positions_of(REQUESTS) * TINY.vocab, 1)
    assert kinds["widen_cell"] == (
        2 * predictions_of(REQUESTS) + positions_of(REQUESTS) * d,
        1,
    )
    for c in range(2, 6):
...
def test_marks_tile_every_gate(run) -> None:
    ...
    assert (
        sum(row.copies * row.size for row in table.rows if row.role == VERIFICATION)
        == n
    )
    assert index.replay_units.count == 1 + len(REQUESTS)
    assert (
        index.verification_unit_count
        == sum(row.copies for row in table.rows if row.role == VERIFICATION)
        == 27_675
    )
...
def test_interfaces_match_enumeration_at_a_smaller_shape(check_interfaces) -> None:
    shape = GPT2Shape(
        layers=1, d_model=16, heads=1, d_ff=16, vocab=5, context=4, argmax_block=2
    )
...
def test_the_request_and_the_weights_are_closed_and_the_decode_steps_are_not(
    run,
) -> None:
    constructor, compiled = run
    table = compiled.kind_table()
    ...
    names = {
        name: rows[digest] for digest, name in constructor.model.kind_names().items()
    }
    ...
    assert (weights.source_weights, weights.out_count, weights.copies) == (
        TINY.weight_count,
        0,
        1,
    )
    for prompt, new in ((3, 3), (2, 2)):
        ...
        assert (
            request.closed
            and request.copies == 1
            and request.input_count == TINY.weight_count
        )
        assert (
            request.out_count == new
            and request.out_bits == request.reach_bits == new * TINY.width
        )
        assert names[f"prefill({prompt})"].closed
    assert (
        not names["decode(4)"].closed
        and not names["decode(5)"].closed
        and not names["decode(3)"].closed
    )
...
def test_no_verification_unit_is_wider_than_an_argmax_block(run) -> None:
    ...
    assert all(
        row.out_bits == cut_bits(row) for row in cells
    )  # the interface, not the reach, is the cut
    sources = table.input_count + table.weight_count
    gates_at = {
        k: sum(r.copies * r.size for r in cells if cut_bits(r) == k) for k in widths
    }
...
def test_compilation_is_deterministic(run) -> None:
    ...
    assert (
        recompiled.digest == compiled.digest
        and recompiled.index.digest == compiled.index.digest
    )
    ...
    assert (
        constructor.manifest["gate_set"] == constructor.gate_set.digest
    )  # the semantics are part of the identity
    # requests of one shape are one kind; the same run in another order is another description
    same = compile_gpt2(
        constructor, (Request((1, 2), 2), Request((3, 4), 2), Request((5, 6), 2))
    )
    units = [
        row for row in same.index.kinds() if row.role == REPLAY and row.out_count > 0
    ]
...
def test_it_takes_no_advice_and_checks_its_requests() -> None:
    ...
    with pytest.raises(TracerError, match="nonempty tuple"):
        constructor((), b"")
    assert constructor.output_layout(REQUESTS) == (
        (0, 0),
        (0, 1),
        (0, 2),
        (1, 0),
...
def test_the_legacy_explicit_dag_agrees_on_inner_products_and_macs() -> None:
    ...
    positions, predictions, keys = (
        positions_of((LEGACY_REQUEST,)),
        predictions_of((LEGACY_REQUEST,)),
        keys_of((LEGACY_REQUEST,)),
    )

    assert (
        kinds[f"dot({d},True,True)"][0]
        == products["q-projection"]
        + products["k-projection"]
        + products["v-projection"]
    )
    assert (
        kinds[f"dot({d},True,False)"][0]
        == products["output-projection"] + products["expansion"]
    )
    assert kinds[f"dot({TINY.d_ff},True,False)"][0] == products["contraction"]
    assert (
        kinds[f"dot({dh},False,False)"][0] == products["score"] + positions * d
    )  # plus the embedding dots
    ...
    projections = (
        kinds[f"dot({d},True,True)"][0] + kinds[f"dot({d},True,False)"][0]
    ) * d
    ...
    assert (
        kinds["scale_cell"][0] == products["score"]
    )  # the scaling is a VU of its own here
    # what the tensor cores actually execute: 16 MACs per step, the mix padded to 16 keys, the one-hot embedding
    steps = sum(
        copies * int(name[4:-1].split(",")[0]) // STEP
        for name, (copies, _) in kinds.items()
        if name.startswith("dot(")
    )
    assert (
        STEP * steps
        == legacy_macs
        + layers * heads * dh * (positions * STEP - keys)
        + positions * d * TINY.vocab_padded
    )
...
def test_gpt2_small_compiles_and_the_table_has_the_documented_numbers(small) -> None:
    ...
    assert len(description) == 1_090_184  # count-one ranges carry stride 0
    assert index.n == 1_924_349_881
    assert index.weight_count == 124_490_068 and index.input_count == 96
    assert (
        index.replay_units.count == 4 and index.verification_unit_count == 177_855_025
    )
    assert len(table.rows) == 291
    assert (
        index.n
        == constructor.gate_budget(SMALL_REQUESTS)["total"]
        + index.input_count
        + index.weight_count
    )
    cells = [row for row in table.rows if row.role == VERIFICATION]
    assert (
        max(row.out_bits for row in cells) == 48
        and max(cut_bits(row) for row in cells) == 48
    )
    computed = table.n - table.input_count - table.weight_count
    at = {
        k: sum(row.copies * row.size for row in cells if cut_bits(row) == k)
        for k in (16, 32, 48)
    }
    ...
    assert (
        requests[0].closed and requests[0].out_bits == requests[0].reach_bits == 32 * 16
    )
    assert requests[0].size == 599_953_271
    budget = constructor.gate_budget(SMALL_REQUESTS)
    assert (
        budget["embedding"] == 465_856_461
        and budget["lm_head"] == 231_584_256
        and budget["mlp"] == 691_504_128
    )
...
def test_gpt2_small_bound_and_cost_fold_over_the_table(small) -> None:
    ...
    strict = bound(
        compiled,
        VerificationPolicy(1, Fraction(3, 4)),
        ETA,
        BoundOptions(knapsack=False),
    )
    assert (
        not strict.capped and 1200 < strict.bits < 1250
    )  # 1224.6: two and a half requests' worth
...
def test_a_thousand_gpt2_small_requests_are_one_repeat_and_the_bound_bites() -> None:
    ...
    constructor = GPT2G(GPT2Shape.small())
    requests = tuple(
        Request(tuple((11 * i + r) % 50257 for i in range(32)), 32) for r in range(1000)
    )
    ...
    assert (
        compiled.circuit.outputs[32]
        == compiled.circuit.Out(compiled.index.replay_units.unit(2))[0]
    )
    loose = bound(
        compiled,
        VerificationPolicy(Fraction(1, 10), Fraction(1, 10)),
        ETA,
        BoundOptions(knapsack=False),
    )
    firm = bound(
        compiled,
        VerificationPolicy(Fraction(1, 2), Fraction(1, 10)),
        ETA,
        BoundOptions(knapsack=False),
    )
