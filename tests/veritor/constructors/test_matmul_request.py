...
def test_matmul_request_compiles_to_a_compiled_circuit() -> None:
    ...
    assert request.public_inputs == (5, 6, 7, 8, 9, 10) and request.weight_values == (
        1,
        2,
        3,
        4,
    ...
    assert (
        tuple(values[o] for o in compiled.circuit.outputs) == request.expected_outputs
    )
...
def test_matmul_marks_rows_as_replay_and_dots_as_verification() -> None:
    ...
    assert (
        index.replay_units.count == 2 + rows
    )  # the activations, the weights, the rows
    assert (
        index.verification_unit_count
        == index.input_count + index.weight_count + rows * columns
    )
