...
def assert_source_domain_matches(domain, expected: list[int], rank_of, n: int) -> None:
    ...
    assert (
        [domain.unrank(rank) for rank in range(domain.count)]
        == expected
        == list(domain)
    )
...
def assert_interfaces_match_enumeration(
    index: Index, circuit: DescriptionCircuit
) -> None:
    flat = FlatCircuit(
        [circuit[address] for address in range(circuit.n)],
    ...
    for node in nodes_below(index.root):
        ...
        assert (
            rows[node.kind].source_inputs
            == definition.input_total
            == len(inputs_inside)
        )
        assert (
            rows[node.kind].source_weights
            == definition.weight_total
            == len(weights_inside)
        )
        base = node.frame.base
        for runs, inside in (
            (definition.input_runs, inputs_inside),
            (definition.weight_runs, weights_inside),
        ):
            members = sorted(
                base + run.element(k) for run in runs for k in range(run.count)
            )
    ...
    assert (circuit.input_count, circuit.weight_count) == (
        len(expected_inputs),
        len(expected_weights),
    )
    assert (index.input_count, index.weight_count) == (
        len(expected_inputs),
        len(expected_weights),
    )
    assert_source_domain_matches(index.inputs(), expected_inputs, circuit.input_rank, n)
    assert_source_domain_matches(
        index.weights(), expected_weights, circuit.weight_rank, n
    )
    assert index.inputs().identity_digest != index.weights().identity_digest

    units = list(index.replay_units)
    boundary = index.boundary()
    expected_boundary = sorted(
        set(expected_inputs).union(*(expected_out[unit] for unit in units))
    )
    ...
    assert [boundary.rank(address) for address in members] == list(
        range(boundary.count)
    )
    assert [
        address for address in range(n) if boundary.contains(address)
    ] == expected_boundary
    # every circuit output is committed: in the boundary, or a weight gate under κ_W
    assert all(
        output in boundary or circuit[output].is_weight for output in circuit.outputs
    )
    ...
    for r, unit in enumerate(units):
        ...
        unit_outputs = {
            a for node in index.verification_units(r) for a in expected_out[node]
        }
        assert (
            set(expected_out[unit]) <= unit_outputs
        )  # the refinement: Out(R) ⊆ ⋃ Out(V)
        expected_interior = [
            a
            for a in unit.interval
            if a in unit_outputs and a not in expected_out[unit]
        ]
        ...
        assert [
            interior.unrank(rank) for rank in range(interior.count)
        ] == expected_interior
        assert [interior.rank(a) for a in expected_interior] == list(
            range(len(expected_interior))
        )
        assert [
            a for a in range(-1, n + 1) if interior.contains(a)
        ] == expected_interior
    ...
    assert rows[index.root.kind].interior_count == sum(
        index.interior(r).count for r in range(len(units))
    )
    ...
    committed = [
        ...
        set(expected_boundary) - set(expected_inputs),
    ]
    committed += [set(iter_domain(index.interior(r))) for r in range(len(units))]
    assert sum(len(part) for part in committed) == len(
        set().union(*committed)
    )  # pairwise disjoint
        ...
        for node in index.verification_units(r):
            for address in node.interval:
                if address in internal:
                    assert (
                        address not in expected_out[node]
                    )  # an internal gate declares nothing
