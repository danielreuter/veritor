...
def assert_same_profile(compiled: KindTable, model: KindTable, io_count: int) -> None:
    assert (
        ...
        compiled.weight_count,
        compiled.replay_unit_count,
    ) == (
        model.n,
        model.input_count,
        model.weight_count,
    ...
    for policy in POLICIES:
        for eta in (Fraction(1, 2), Fraction(1, 100)):
            assert bound(compiled, policy, eta).bits == pytest.approx(
                bound(model, policy, eta).bits, abs=1e-9
            )
        assert cost(compiled, policy) == cost(model, policy)
        assert expected_work(compiled, policy, io_count) == expected_work(
            model, policy, io_count
        )


@pytest.mark.parametrize("lm, serving", SHAPES, ids=("toy", "deeper"))
def test_request_row_is_the_profile_of_requests_g(
    lm: LMShape, serving: ServingShape
) -> None:
    requests = tuple(
        Request(tuple(range(serving.prompt)), serving.generated)
        for _ in range(serving.requests)
    )
    ...
    assert_same_profile(
        compiled.kind_table(), model, serving.input_count + serving.output_count
    )
    assert compiled.kind_table().digest == compiled.digest != model.digest


@pytest.mark.parametrize("lm, serving", SHAPES, ids=("toy", "deeper"))
def test_step_row_is_the_profile_of_cluster_g_on_identical_requests(
    lm: LMShape, serving: ServingShape
) -> None:
    requests = tuple(
        Request(tuple(range(serving.prompt)), serving.generated)
...
def test_every_partition_tiles_the_same_gates() -> None:
    ...
    for table in tables.values():
        for role in (REPLAY, VERIFICATION):
            assert (
                sum(row.copies * row.size for row in table.rows if row.role == role)
                == table.n
            )
        assert sum(row.copies for row in table.rows if row.role == VERIFICATION) == sum(
            row.copies * row.verification_units
            for row in table.rows
            if row.role == REPLAY
        )
...
def test_finer_replay_levels_have_more_units_and_the_cell_level_one_word_each() -> None:
    counts = {
        levels: serving_table(TOY, *levels).replay_unit_count for levels in partitions()
    }
    ...
    widest = max(
        row.out_bits
        for row in serving_table(TOY, "cell", "gate").rows
        if row.role == REPLAY
    )
...
def test_verification_levels_refine() -> None:
    tables = [
        serving_table(TOY, "request", level)
        for level in ("layer", "row", "cell", "gate")
    ]

    units = [
        sum(r.copies for r in table.rows if r.role == VERIFICATION) for table in tables
    ]
    ...
    assert (
        max(row.out_bits for row in tables[2].rows if row.role == VERIFICATION)
        == TOY.width
    )
...
def test_closed_kinds_are_the_weights_the_root_the_requests_and_the_prefills() -> None:
    ...
    for levels in partitions():
        ...
        assert replay == {
            row.kind for row in table.rows if row.role == REPLAY and not row.input_count
        } | (closed & {row.kind for row in table.rows if row.role == REPLAY})
        # everything reading an activation, a token or the cache is open
        assert not any(
            row.closed for row in table.rows if row.kind.startswith("('decode")
        )
        assert not any(
            row.closed for row in table.rows if row.kind.startswith("('matvec")
        )
...
def test_reach_is_the_requests_tokens_or_the_rest_of_the_wave() -> None:
    ...
    for levels in partitions():
        ...
        if levels[0] == "step":
            ...
            for c in range(TOY.prompt + 1, TOY.prompt + TOY.generated):
                ...
                assert (
                    step.reach_bits
                    == TOY.batch * (TOY.prompt + TOY.generated - c) * TOY.width
                    < step.out_bits
                )
    ...
    assert (
        "cell",
        "gate",
    ) not in wide_partitions  # no RU or VU wider than a word there


def test_the_shape_is_checked() -> None:
    with pytest.raises(ValueError, match="multiple of heads"):
        ServingShape(
            vocab=8, d_model=6, heads=4, layers=1, prompt=1, generated=1, requests=1
        )
    ...
    with pytest.raises(ValueError, match="positive integer"):
        ServingShape(
            vocab=8, d_model=4, heads=2, layers=0, prompt=1, generated=1, requests=1
        )
